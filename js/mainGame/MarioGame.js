...
function MarioGame() {
  ...
  this.checkElementMarioCollision = function(element, row, column) {
    ...
    if (collisionDirection == 'l' || collisionDirection == 'r') {
      ...
      if (element.type == 5) {
        //flag pole
        if (mario.num_powerups < 4) {
          gotNotdone = true;
          $('#notdone-message').show();
          $('#notdone-enter').blink({delay: 500});
        } else {
      ...
      if (element.type == 3) {
        ...
        //   powerUps.push(powerUp);
        // }
        if (mario.num_powerups == 0) {
          powerUp.mushroom(element.x, element.y);
          powerUps.push(powerUp);
        } else if (mario.num_powerups == 1) {
          powerUp.flower(element.x, element.y);
          powerUps.push(powerUp);
        } else if (mario.num_powerups == 2) {
          powerUp.lightbulb(element.x, element.y);
          powerUps.push(powerUp);
        } else {
          powerUp.hatdegree(element.x, element.y);
          powerUps.push(powerUp);
        }
    ...
    for (var i = 0; i < powerUps.length; i++) {
      ...
      if (collWithMario) {
        ...
        if (mario.num_powerups == 0) {
          ...
          var cur_height = $( '#flower-message' ).height();
          var cur_width = $( '#flower-message' ).width();
          var screen_off = $( '#play-inner-screen-2' ).offset();
          $( '#flower-message' ).css({
            top: screen_off.top + ($( '#play-inner-screen-2' ).height() - cur_height) / 4,
            left: screen_off.left + ($( '#play-inner-screen-2' ).width() - cur_width) / 2,
          ...
          $('#mushroom-message').hide();
          $('#flower-message').hide();
          $('#lightbulb-message').show();

          var cur_height = $( '#lightbulb-message' ).height();
          var cur_width = $( '#lightbulb-message' ).width();
          var screen_off = $( '#play-inner-screen-2' ).offset();
          $( '#lightbulb-message' ).css({
            top: screen_off.top + ($( '#play-inner-screen-2' ).height() - cur_height) / 4,
            left: screen_off.left + ($( '#play-inner-screen-2' ).width() - cur_width) / 2,
          });

          $('#lightbulb-enter').blink({delay: 500});
        } else {
          gotMushroom = false;
          gotFlower = false;
          gotLightbulb = false;
          ...
          $('#mushroom-message').hide();
          $('#flower-message').hide();
          $('#hatdegree-message').show();

          var cur_height = $( '#hatdegree-message' ).height();
          var cur_width = $( '#hatdegree-message' ).width();
          var screen_off = $( '#play-inner-screen-2' ).offset();
          $( '#hatdegree-message' ).css({
            top: screen_off.top + ($( '#play-inner-screen-2' ).height() - cur_height) / 4,
            left: screen_off.left + ($( '#play-inner-screen-2' ).width() - cur_width) / 2,
          });

          $('#hatdegree-enter').blink({delay: 500});
        }

        mario.num_powerups += 1;
        powerUps.splice(i, 1);
    ...
    if (marioInGround) {
      ...
      if (tickCounter > maxTick) {
        ...
        timeOutId = setTimeout(function() {
          ...
          gotHatdegree = false;
          gotNotdone = false;
          $('#play-inner-screen-2').css({backgroundColor: 'white'});
          $('#character-celebrate').css({'top': '40%', 'left': '45%'});
          $('#character-celebrate').show();
          $('#enter-intro-btn2').blink({delay: 500});
          $( '#character-celebrate' ).blink({delay: 500});
          $('#end-text').css('color', 'black');
          $('#end-text').show();
          completedEverything = true;
        }, 1000);
      }
    }
  };
